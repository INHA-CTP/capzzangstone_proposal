...
과거 20%의 주 품목들이 전체 매출의 80%를 차지하였다.
그러나 점차 개인 성향이 중시되며, 인터넷의 성장으로 정보의 접근이 쉬워지자 나머지 80%의 비주류 품목의 전체 매출이 무시 못할 규모가 되었다. 최근 IT 기업들은 개인의 성향에 중시하여 추천 알고리즘을 통해 시스템을 구축하기 시작했고, 그 결과 괄목할만한 결과를 얻어내었다.
이에 평소 알고리즘에 관심이 많았던 캡짱스톤 조는 강의 추천 시스템을 구현해 추천 알고리즘을 실제 적용을 해보고자 한다.
...
- 기존 프로젝트와 제안 내용과의 차이 및 기존 문제 해결 방안
    ...
    * 사용이 간편한 UI / UX
- 기술적인 어려움
    * 추천해주는 과목(item)이 적절하게 추천이 되었는지, 추천을 받는 user의 만족도를 수량적으로 판단하기 어려움
        * 이 부분에 대해서는 자체 추천 알고리즘 소개 논문에서 성과 부분을 보면 도움이 될 것이라 예상
    * 특정 OS 기반이 아닌 브라우저 기반으로 어려운 디바이스 최적화
    * 모든 학생이 사용해야 하기 때문에 모든 브라우저와 호환 필요
...
* 개발 플랫폼 : **Web Application** <br/>
    ...
    2. 자바스크립트 기반으로 만들어지기 때문에 차후에 스마트폰 어플리케이션이나 데스크탑 어플리케이션으로 만들기 수월하다.=> 높은 확장성<br/>
    3. 상대적으로 많은 라이브러리와 자료 존재<br/>
    4. 서버 언어로 `Node.js`를 사용하기 때문에 클라이언트와 동시 개발하기 수월하다.<br/>
- 상세 요구사항
    * 기능적 요구사항 (functional requirements)
        * 과목 recommendation systems
        * 시간표 제작
        * 모바일 환경을 고려한 반응형 웹으로 제작
        * 기존보다 쉽고 간편한 검색
    * 비기능적 요구사항 (non-functional requirements)
        * 해당 과목에 관심이 있는 인원 명시를 통한 경쟁 가능성 제시로 강의 분산
        * 추천 시스템 사용 유도
