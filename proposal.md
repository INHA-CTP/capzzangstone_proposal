...
- 기존 프로젝트와 제안 내용과의 차이 및 기존 문제 해결 방안
    ...
    * 사용이 간편한 UI / UX
- 기술적인 어려움
    * 추천해주는 과목(item)이 적절하게 추천이 되었는지 추천을 받는 user의 만족도를 수량적으로 판단의 어려움
        * 이 부분에 대해서는 자체 추천 알고리즘 소개 논문에서 성과 부분을 보면 도움이 될 것이라 예상
    * 특정 OS 기반이 아니라 브라우저 기반이기 때문에 디바이스 최적화 문제의 어려움
    * 모든 학생이 사용해야 하기 때문에 모든 브라우저에 대응 필요
    * 특정 기간에 급격히 늘어나는 요청에 대한 대응
