...
- 상세 요구사항
    * 기능적 요구사항 (functional requirements)
        * 메인 페이지 구성
            * 로그인-아이디, 시간표, 평가하기
            * 시간표
                * 일반적인 모양의 시간표와 내가 선택한 과목들이 보여지는 화면
                * 내가 지금 몇 학점을 선택했는지
                * 학기와 년도를 지정할 수 있어야 함
                * 몇 학점을 선택했는지
                * 클리어 버튼
                * 박스에는 과목명, 강의실 나타냄
                * 강의명이 길 경우 어떻게 나타낼 지, 학생들 사이에서 통용되는 말로 입력 되었을시(ex> 오퍼레이팅 시스템->오에스)
                * 시간표에 표시된 박스 클릭 하면 새로운 모달이 올라와야 한다.
                * 모달 내용
                    * 삭제버튼
                    * 강의정보
                    * 몇명이 관심있는지
                    * 별점 / 평가 댓글들(optional)
                    * 새로운 시간표 추가하기(like everytime)
                    * 교양 추천 받으러 가기  / 전공 추천 받으러 가기 버튼
            * 검색
                * 학과 선택
                * 추천 강의들을 일부만 보여주고, 그 검색창 내에서 클릭 하면 나머지 리스트를 보여준다.
        * 개인정보 페이지
            * 프로필 편집
            * 강의 평가 / 평가 추가하기
            * 사용자 분석(성향) - 선호도
            * 댓글 / 내가 평가한 리스트 보여주기
            * 자신의 시간표를 공유하는 기능(optional)
        * 강의 평가하기 뷰
            * 처음 로그인 했을 경우 이 화면이 첫 화면 강제
            * 인피니트 스크롤
            * 코멘트
        * 관리자 페이지
            * 크롤링 기능
